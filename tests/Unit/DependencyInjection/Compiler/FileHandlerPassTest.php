...
final class FileHandlerPassTest extends TestCase
{
    ...
    public function testFailsWhenFileHandlerServiceIsMissing(): void
    {
        $container = new ContainerBuilder();
        $container->setParameter('nowo_yopass.file_handler', 'missing.handler');

        $this->expectException(InvalidConfigurationException::class);
        $this->expectExceptionMessage('missing.handler');

        (new FileHandlerPass())->process($container);
    }

    public function testNoOpWhenFileHandlerParameterIsNull(): void
    {
        $container = new ContainerBuilder();
        $container->setParameter('nowo_yopass.file_handler', null);
